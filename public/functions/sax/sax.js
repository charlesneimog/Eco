...
function audioFftYin(target_frequency) {    
    ...
    var TargetMcModule = Math.round(Target_mc % 1200);
    document.getElementById("InstrumentTitle").innerHTML = "Tunning started | Target: " + TargetMcModule + " mc";
    ...
    function ckn_fft(dataArray, sampleRate){ // sorry, I do not know how to name this
        ...
        send2pd("SaxPeaks", array_FrekAndAmp);
    }
    // -----------------------------------------------
    function PitchDetection(data, sampleRate) {
        var now = new Date().getTime();
        // get the duration of the fftSize Windows
        var durationOfFftSize = (1 / sampleRate * fftSize) * 1000 ;
        if (now - lastYin > (durationOfFftSize + 1) && executingTune == true && yinExecuted == false) {
            lastYin = now;
            // NOTE: Here is where the C code is called
            // =======================================
            const precisionOfPitchDetection = 0.3;
            ...
            var pitch = yinModule._C_getPitch(sound, sampleRate, precisionOfPitchDetection); // Here we call the C function, higher the 
            yinModule._freeMemory(sound); // Here we free the memory
            // ========================================
            if (pitch > 3520 || pitch < 65){ // if detented pitch is out of range
                noPitchesDetected += 1;
                if (noPitchesDetected > 35) {
                    pitchesArray = [];
                    document.getElementById("InstrumentTitle").innerHTML = "No pitch detected!";
                ...
                noPitchesDetected = 0;
                if (pitchesArray.length > 5){
                    pitchesArray.shift(); // NOTE: this make the array to have always 10 elements
                    ...
                    if (Math.abs(diff) > 100){
                        document.getElementById("InstrumentTitle").style.backgroundColor =  "red";
                        if (diff > 0){
                            document.getElementById("InstrumentTitle").innerHTML = midicents + " | " + "target: " + TargetMcModule;
                        }
                        else{
                            document.getElementById("InstrumentTitle").innerHTML = midicents  + " | " + "target: " + TargetMcModule;
                        }
    ...
    function AudioStream() {
        ...
        analyser.fftSize = fftSize; // this will 
        analyser.sampleRate = 44100;
        ...
        function step() {
            ...
            if (executingTune == false) {
                audioContext.close();
                document.getElementById("InstrumentTitle").innerHTML = "Sax";
...
function updateMeasureBarProgress(measureDuration, startPlay, tupletDuration, frequencyTarget, gestureNumber, scoreImage){
    ...
    var interval = setInterval(function() {
        ...
        if (div_width == 100) {
            ...
            send2pd("sax", "gesto" + gestureNumber + " " + gestureNumber); 
            if (gestureNumber != 0){
...
function metronome4Attack(span, startPlay, tupletDuration, frequencyTarget, gestureNumber, gestRepetition, gestProb){
    ...
    var interval = setInterval(function() {
        ...
        if (gestRepeatRandom > (1 - gestProb) && fastGestureAlreadyPlayed == false && measureDivWidth > 33){
            ...
            send2pd("fastGest", "fastGest in gesture" + gestureNumber + " for Sax");
        ...
        if (div_width >= 100) {
            ...
            if (measureFinished === false){
                metronome4Attack(span, startPlay, tupletDuration, frequencyTarget, gestureNumber, gestRepetition, gestProb);
            } 
            else{
...
function updateScore(i) {
    ...
    readTextFile("sax.json" + '?' + new Date().getTime(), function(text){
